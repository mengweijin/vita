package com.github.mengweijin.vita.framework.log.logback;
...
@Component
@AllArgsConstructor
public class DbLoggerAppender extends UnsynchronizedAppenderBase<ILoggingEvent> implements InitializingBean {

    /**
     * 白名单。这些日志名称的日志不会被记录到数据库。
     */
    private static final Set<String> LOGGER_NAME_IGNORE_SET = Set.of(
            "p6spy",
    ...
    private static final String TAB = StrUtil.fillAfter(Const.EMPTY, ' ', TAB_LENGTH);
    ...
    private void recordLog(ILoggingEvent event) {
        try {
            ...
            boolean anyMatch = LOGGER_NAME_IGNORE_SET.stream().anyMatch(i -> i.equalsIgnoreCase(loggerName));
            if (anyMatch) {
                return;
            }
            ...
            logSystemDO.setFormattedMessage(CharSequenceUtil.subPre(event.getFormattedMessage(), MAX_LENGTH));
            ...
            logSystemService.saveBySingleExecutorService(logSystemDO);
