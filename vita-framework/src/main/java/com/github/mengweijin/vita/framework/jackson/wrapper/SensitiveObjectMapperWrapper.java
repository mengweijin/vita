package com.github.mengweijin.vita.framework.jackson.wrapper;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.databind.DeserializationFeature;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.fasterxml.jackson.databind.SerializationFeature;
import com.fasterxml.jackson.databind.ser.SerializerFactory;
import com.github.mengweijin.vita.framework.jackson.JacksonConfig;
import com.github.mengweijin.vita.framework.jackson.wrapper.sensitive.modifier.SensitiveBeanSerializerModifier;

import java.text.SimpleDateFormat;

/**
 * @author mengweijin
 * @since 2022/5/17
 */
public class SensitiveObjectMapperWrapper extends AbstractObjectMapperWrapper {

    @SuppressWarnings({"java:S2386"})
    public static final String[] SENSITIVE_KEY = new String[]{"password", "pwd", "token", "key"};

    public static final String HIDE_VALUE = "********";

    private static volatile SensitiveObjectMapperWrapper instance;

    private SensitiveObjectMapperWrapper(ObjectMapper objectMapper) {
        super(objectMapper);
    }

    public static SensitiveObjectMapperWrapper getInstance() {
        if (instance == null) {
            synchronized (SensitiveObjectMapperWrapper.class) {
                if (instance == null) {
                    ObjectMapper objectMapper = createSensitiveObjectMapper();
                    instance = new SensitiveObjectMapperWrapper(objectMapper);
                }
            }
        }
        return instance;
    }

    private static ObjectMapper createSensitiveObjectMapper() {
        ObjectMapper mapper = new ObjectMapper();
        //只序列化对象值不为 null 的属性
        mapper.setDefaultPropertyInclusion(JsonInclude.Include.NON_NULL);
        //反序列化的时候如果多了其他属性,不抛出异常
        mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        //如果是空对象的时候,不抛异常
        mapper.configure(SerializationFeature.FAIL_ON_EMPTY_BEANS, false);
        //取消时间的转化格式,默认是时间戳,可以取消,同时需要设置要表现的时间格式
        mapper.configure(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS, false);
        mapper.setDateFormat(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"));
        // 注册 javaTimeModule
        mapper.registerModule(JacksonConfig.javaTimeModule());
        // 重新设置 Jackson 的 Bean 序列化修改器
        SerializerFactory serializerFactory = mapper.getSerializerFactory().withSerializerModifier(new SensitiveBeanSerializerModifier());
        mapper.setSerializerFactory(serializerFactory);
        return mapper;
    }

}
