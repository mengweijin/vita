...
public class GeneratorService {
    ...
    public ContentVO generate(GeneratorBO bo) {
        TemplateVO template = templateService.getTemplateById(bo.getTemplateId());
        TableInfo tableInfo = this.findTableInfoByName(bo.getTableName());
        String fileName = replaceFileName(template.getName(), bo, tableInfo);
        Map<String, Object> objectMap = GeneratorUtils.getObjectMap(bo, tableInfo);

        String content = velocityTemplateEngine.write(fileName, template.getContent(), objectMap);

        ContentVO contentVO = new ContentVO();
        contentVO.setTemplateName(template.getName());
    ...
    private String replaceFileName(String fileName, GeneratorBO args, TableInfo tableInfo) {
        Map<String, Object> objectMap = GeneratorUtils.getObjectMap(args, tableInfo);
