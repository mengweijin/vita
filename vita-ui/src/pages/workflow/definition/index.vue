...
<template>
  ...
  <el-form ref="queryFormRef" :model="queryParams" :inline="true" @submit.prevent="loadTableData">
    ...
    <el-form-item prop="isPublish" label="是否发布">
      <VtSelectDict v-model="queryParams.isPublish" :code="'vt_warmflow_publish'" :style="'width: 120px;'" />
    </el-form-item>
    <el-form-item prop="activityStatus" label="激活状态">
      <VtSelectDict v-model="queryParams.activityStatus" :code="'vt_warmflow_activity_status'"
        :style="'width: 120px;'" />
    </el-form-item>
    <el-form-item>
  ...
  <div class="vt-table">
    <el-table ref="tableRef" v-loading="loading" :data="tableData" :size="size" row-key="id" height="100%" stripe border
      ...
      <el-table-column v-if="columns.flowName.visible" prop="flowName" label="流程名称" min-width="100" />
      <el-table-column v-if="columns.flowCode.visible" prop="flowCode" label="流程编码" width="180" />
      <el-table-column v-if="columns.modelValue.visible" prop="modelValue" label="设计器模型" width="110" align="center">
        <template #default="{ row }">
          <VtTagDict :code="'vt_warmflow_designer_model'" :value="row.modelValue" :size="size"></VtTagDict>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.categoryName.visible" prop="categoryName" label="流程分类" width="100" />
      <el-table-column v-if="columns.version.visible" prop="version" label="版本" width="80" align="center" />
      <el-table-column v-if="columns.isPublish.visible" prop="isPublish" label="是否发布" width="100" align="center">
        <template #default="{ row }">
          <VtTagDict :code="'vt_warmflow_publish'" :value="row.isPublish" :size="size"></VtTagDict>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.formCustom.visible" prop="formCustom" label="表单是否自定义" width="130" align="center">
        <template #default="{ row }">
          <VtTagDict :code="'vt_warmflow_form_custom'" :value="row.formCustom" :size="size"></VtTagDict>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.formPath.visible" prop="formPath" label="表单路径" min-width="100" />
      <el-table-column v-if="columns.activityStatus.visible" prop="activityStatus" label="激活状态" width="100"
        align="center">
        <template #default="{ row }">
          <VtTagDict :code="'vt_warmflow_activity_status'" :value="row.activityStatus" :size="size"></VtTagDict>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.listenerType.visible" prop="listenerType" label="监听器类型" min-width="100" />
      <el-table-column v-if="columns.listenerPath.visible" prop="listenerPath" label="监听器路径" min-width="100" />
      <el-table-column v-if="columns.ext.visible" prop="ext" label="扩展字段" min-width="100" />
      <el-table-column v-if="columns.delFlag.visible" prop="delFlag" label="删除标记" min-width="100" />
      <el-table-column v-if="columns.tenantId.visible" prop="tenantId" label="租户ID" min-width="100" />
            ...
            <el-tooltip content="删除" placement="top">
              <div style="display: inline-block;">
                <el-popconfirm placement="left" width="400" :title="`确定删除【${scope.row.name}】吗？`"
                  ...
                  <template #reference>
                    <el-button type="danger" text :size="size" v-permission="'workflow:flowDefinition:remove'">
                      <template #icon>
                        <el-icon :size="size">
                          <Icon icon="ep:delete"></Icon>
                        </el-icon>
  ...
  <FlowDefinitionEdit ref="flowDefinitionEditRef" @refresh-table="loadTableData"></FlowDefinitionEdit>
</template>
