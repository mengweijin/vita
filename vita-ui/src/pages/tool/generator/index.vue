...
<template>
  ...
  <div class="vt-table">
    <el-table ref="tableRef" v-loading="loading" :data="tableData" :size="size" row-key="id" height="100%" stripe border
      ...
      <el-table-column v-if="columns.name.visible" prop="name" label="表名称" width="330" fixed="left">
        <template #default="{ row }">
          <a href="javascript:" @click="handleOpenTemplateDialog(row)">{{ row.name }}</a>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.havePrimaryKey.visible" prop="havePrimaryKey" label="是否有主键" width="100"
        align="center">
        <template #default="{ row }">
          <VtTagDict :code="'vt_yes_no'" :value="row.havePrimaryKey === true ? 'Y' : 'N'" :size="size"></VtTagDict>
        </template>
      </el-table-column>
      <el-table-column v-if="columns.fieldNames.visible" prop="fieldNames" label="字段名称" min-width="200" />
      <el-table-column v-if="columns.comment.visible" prop="comment" label="表注释" min-width="100" />
      <el-table-column v-if="columns.operation.visible" label="操作" fixed="right" width="80">
        <template #default="scope">
          <div>
            <el-tooltip content="生成代码" placement="top">
              <el-button type="primary" text :size="size" @click="handleOpenTemplateDialog(scope.row)"
