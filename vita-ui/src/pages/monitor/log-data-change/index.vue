...
<template>
  ...
  <el-form ref="queryFormRef" :model="queryParams" :inline="true" @submit.prevent="loadTableData">
    <el-form-item prop="tableName" label="表名称">
      <el-select v-model="queryParams.tableName" clearable :style="'width: 240px;'" placeholder="请选择">
  ...
  <div class="vt-table">
    <el-table ref="tableRef" v-loading="loading" :data="tableData" :size="size" row-key="id" height="100%" stripe border
      ...
      <el-table-column v-if="columns.tableName.visible" prop="tableName" label="表名称" min-width="100" fixed="left" />
      <el-table-column v-if="columns.businessId.visible" prop="businessId" label="业务数据 ID" min-width="100" />
      <el-table-column v-if="columns.readableMessages.visible" prop="readableMessages" label="变更信息" min-width="180" />
            ...
            <el-tooltip content="删除" placement="top">
              <div style="display: inline-block;">
                <el-popconfirm placement="left" width="400" :title="`确定删除账号为【${scope.row.username}】的登录记录吗？`"
                  ...
                  <template #reference>
                    <el-button type="danger" text :size="size" v-permission="'monitor:logDataChange:remove'">
                      <template #icon>
                        <el-icon :size="size">
                          <Icon icon="ep:delete"></Icon>
                        </el-icon>
