...
const extendUtils = {
	 ...
	 * @returns
	 */
	addFullPath: (list, { idKey = "id", parentKey = "parentId", pathKey = "name", separator = "/" }) => {
		const buildPath = (id) => {
			const pathList = [];
			let row = XEUtils.find(list, (item) => XEUtils.toValueString(id) === XEUtils.toValueString(item[idKey]));
			while (row != null) {
