...
export const roleApi = {
  addUsers: (roleId, userIds) =>
    http.post(`${URL_PREFIX}/add-users/${roleId}/${userIds}`),
  ...
  getRoleIdsByUserId: (userId) =>
    http.get(`${URL_PREFIX}/get-role-ids-by-user-id/${userId}`, {}),
  ...
  removeByRoleIdInUserIds: (roleId, userIds) =>
    http.post(
      `${URL_PREFIX}/removeByRoleIdInUserIds/${roleId}/${userIds}`,
      null,
      { loading: false }
    ),

  setPermission: (id, menuIdList = []) =>
    http.post(`${URL_PREFIX}/set-permission`, {
      menuIds: menuIdList,
      roleId: id,
    }),
