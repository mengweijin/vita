...
|  部门管理  |   登录日志    |   缓存过期   |   流程监控   | 二级认证组件  |
|  岗位管理  |   操作日志    |   接口防抖   |   流程实例   | 用户选择组件  |
|  用户管理  |   系统日志    |   全局异常   |   待办任务   | 角色选择组件  |
|  角色管理  |  数据变动日志   |   数据权限   |   我的任务   | 岗位选择组件  |
|  分类管理  |   接口文档    |  配置热刷新   |   我发起的   | 部门选择组件  |
|  字典管理  |   代码生成器   | 自定义数据验证  |   我的待办   | 字典标签组件  |
|  配置管理  |    国际化    | SSE 消息推送 |   我的已办   | 字典选择组件  |      
|  文件管理  | 数据存储自动加解密 |   二级认证   |   抄送我的   | 权限控制指令  |
